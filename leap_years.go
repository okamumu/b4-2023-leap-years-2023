package main

// # Leap Years
//
// Write a function that returns true or false depending on whether its input integer is a leap year or not.
// A leap year is defined as one that is divisible by 4, but is not otherwise divisible by 100 unless it is
// also divisible by 400.
//
// For example, 2001 is a typical common year and 1996 is a typical leap year,
// whereas 1900 is an atypical common year and 2000 is an atypical leap year.

// leapYears
// A function to return a boolean value indicating whether a given year `x` is a leap year or not.
//
// The argument `x`: an integer indicating a year
// The retuned value: a boolean value

func leapYears(y int) bool {
	return true
}
